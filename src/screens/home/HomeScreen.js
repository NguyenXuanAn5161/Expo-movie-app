...
const HomeScreen = () => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerLeft: () => <Text>Hello bạn!</Text>,
      headerStyle: {
      ...
      headerRight: () => (
        ...
        >
          ...
          <Ionicons
            onPress={() => navigation.navigate("Địa điểm")}
            name="location-outline"
            size={24}
            color="black"
          />
          <Pressable onPress={() => navigation.navigate("Địa điểm")}>
            <Animated.Text
              style={[
                styles.text,
  ...
  const handleBottomModal = (isShow) => {
  ...
  return (
    <View>
      ...
      />
      <Pressable
        onPress={() => handleBottomModal(true)}
        style={{
      ...
      <BottomModal
        onBackdropPress={() => handleBottomModal(false)}
        swipeDirection={["up", "down"]}
        swipeThreshold={200}
        footer={
          <ModalFooter>
            <Pressable
              onPress={() => handleApplyFilter(selectedFilter)}
              style={{
                paddingRight: 10,
                marginLeft: "auto",
                marginRight: "auto",
        ...
        modalTitle={<ModalTitle title="Bộ lọc" />}
        modalAnimation={new SlideAnimation({ slideFrom: "bottom" })}
        visible={modalVisible}
        // nút quay về của điện thoại
        onHardwareBackPress={() => handleBottomModal(false)}
        onTouchOutside={() => handleBottomModal(false)}
      >
          ...
          >
            {languages.map((item, index) =>
              selectedFilter === item.language ? (
                ...
                <Pressable
                  onPress={() => handleSelectedFilter(item.language)}
                  key={index}
                  style={{
                    margin: 10,
                    borderWidth: 1,
