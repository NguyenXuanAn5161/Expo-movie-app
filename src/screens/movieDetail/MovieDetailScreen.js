...
const MovieDetailScreen = () => {
  ...
  return (
    <View>
      ...
      {malls
        .filter((item) => item.place === selectedLocation)
        .map((item) =>
          item.galleria.map((multplex, index) => (
            <Pressable
              key={index}
              onPress={() => handleMall(multplex.name)}
              style={{ marginHorizontal: 20, marginVertical: 10 }}
            >
              <Text style={{ fontSize: 15, fontWeight: "500" }}>
                {multplex.name}
              </Text>
              {mall.includes(multplex.name) ? (
                <FlatList
                  numColumns={3}
                  data={multplex.showtimes}
                  renderItem={({ item }) => (
                    <Pressable
                      style={{
                        borderColor: "green",
